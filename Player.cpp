...
{
    if (starship == nullptr || starship->hpNow <= 0) return;

    // trừ vào Khiên trước nếu Player đang bật khiên
    if (hasShield && shieldHp > 0.0f) {
        if (shieldHp >= damage) {
            shieldHp -= damage; // Khiên gánh toàn bộ dame
            damage = 0.0f;
        } else {
            damage -= shieldHp;
            shieldHp = 0.0f;
    ...
    // Nếu còn dame dư thừa (hoặc không có khiên), trừ vào máu thật của máy bay
    if (damage > 0.0f) {
        starship->hpNow -= damage;
        if (starship->hpNow < 0.0f) {
            starship->hpNow = 0.0f;
        }
    }
}
...
void Player::Update(float deltaTime, WeaponSystem& weaponSystem, EffectSystem& effectSystem)
{
    ...
    effectSystem.FollowHealing(*starship,hasHealing);
    ...
    {
    // lấy loại đạn đang đổi (nếu còn time) hoặc đạn gốc của tàu
